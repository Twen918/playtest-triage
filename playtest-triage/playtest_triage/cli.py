...
from .dedup import merge_similar
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--dedup",
        action="store_true",
        help="merge near-duplicate feedback before classifying",
    )
    parser.add_argument(
        "--dedup-threshold",
        type=float,
        default=0.8,
        help="similarity threshold for --dedup, between 0 and 1 (default 0.8)",
    )
    parser.add_argument(
        "--mock",
        action="store_true",
    ...
    if args.dedup:
        if not 0 < args.dedup_threshold <= 1:
            parser.error("--dedup-threshold must be between 0 and 1")
        merged = merge_similar(items, threshold=args.dedup_threshold)
        if len(merged) < len(items):
            print(f"Merged near-duplicates: {len(items)} -> {len(merged)} items")
        counts = [count for _, count in merged]
        items = [text for text, _ in merged]
    ...
    if counts:
        for finding, count in zip(findings, counts):
            finding.count = count
